...
class Agent:
    def __init__(self, gamma: float, lr_actor: float, lr_critic: float, fc1_size, fc2_size, save_path: str):
        ...
        self.actor_local = Actor(INPUT_SIZE, ACTION_SIZE, fc1_size, fc2_size).to(device).eval()
        self.actor_target = copy.deepcopy(self.actor_local).to(device).eval()
        self.critic_local = (
            Critic(INPUT_SIZE, ACTION_SIZE, 1, fc1_size, fc2_size).to(device).eval()
        )  # 1 since we output reward directly
    ...
    def update(self, experiences: list[tuple[list[float], int, list[float], float, int]]):
        ...
        # TODO do we want gradients from actor to backpropagate to critic?
